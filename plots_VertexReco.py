...
test_y = df00['Gridpoint']
y_predicted = df00['Predicted_Gridpoint']

# data = df00['Gridpoint']
# dataprev = df00['Predicted_Gridpoint']
...
ax1.scatter(test_y,y_predicted)
ax1.plot([test_y.min(),test_y.max()],[test_y.min(),test_y.max()],'k--',lw=3)
ax1.set_xlabel('Gridpoint')
ax1.set_ylabel('Predicted (Gridpoint)')
ax1.set_xlim(0.,3000.)
ax1.set_ylim(0.,3000.)
#plt.show()
fig1.savefig('gridplot.png')
plt.close(fig1)

canvas = ROOT.TCanvas()
canvas.cd(1)
th2f = ROOT.TH2F("Gridpoint", "; Predicted Gridoint", 100, 0, 3000., 100, 0., 3000.)
for i in range(len(test_y)):
    th2f.Fill(test_y[i], y_predicted[i])
line = ROOT.TLine(0.,0.,3000.,3000.)
th2f.SetStats(0)
th2f.Draw("ColZ")
line.SetLineColor(2)
canvas.Draw()
line.Draw("same")
canvas.SaveAs("Scatter Grid.png")
